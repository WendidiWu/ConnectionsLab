...
button.addEventListener('click', function(){
    ...
    .then(data => {
        ...
        eventYear5.innerHTML = events[Math.floor(events.length/5*4)].year;

        let eventHeading5 = document.getElementById('event_heading_5');
        eventHeading5.innerHTML = events[Math.floor(events.length/5*4)].text;
 
        let eventPara5 = document.getElementById('event_para_5');
        eventPara5.innerHTML =events[Math.floor(events.length/5*4)].pages[0].extract_html;

        let eventImage5 = document.getElementById('event_image_5');
        eventImage5.src = events[Math.floor(events.length/5*4)].pages[0].thumbnail.source;
